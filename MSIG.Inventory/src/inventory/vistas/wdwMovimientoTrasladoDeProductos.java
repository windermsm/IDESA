...
public class wdwMovimientoTrasladoDeProductos extends javax.swing.JInternalFrame {
    ...
    private void descontarProducto(String pId_producto, float pCantidad){
        ...
        try{
            BuscarProducto = acceso_producto.buscarProducto(pId_producto, Inventory.lblUsuario.getText(), Inventory.lblTerminal.getText());
            if(BuscarProducto.isEmpty()){
                Mensaje.manipulacionExcepciones("critico", "No existe el Producto " + pId_producto + " en la base de datos.", "Buscar Producto");
            } else {
                if(BuscarProducto.size()>1){
                    Mensaje.manipulacionExcepciones("critico", "Existe mas de un Producto con este ID " + pId_producto, "Buscar Producto");
                } else {
                    //PROCEDE A REALIZAR LA ACTUALIZACION DESCONTANDO LA CANTIDAD
                    acceso_producto.actualizarExistencia(pId_producto, BuscarProducto.get(0).getExi_producto() - pCantidad, Inventory.lblUsuario.getText(), Inventory.lblTerminal.getText());
                }
            }
    ...
    private void obtenerDatosSucursal(String id){
        if(txtIdSucursal.getText().equals("") || txtIdSucursal.getText().isEmpty()) {
            Mensaje.manipulacionExcepciones("critico", "Ingrese el codigo de sucursal", "Obtener Datos Sucursal");
        } else {
            sucursal = acceso_sucursal.buscarSucursales(Integer.parseInt(id), Inventory.lblUsuario.getText(), Inventory.lblTerminal.getText());
            if (sucursal == null) {
                txtNombreSucursal.setText("");
                txtDireccion.setText("");
                txtDireccionTraslado.setText("");
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
            ...
            if (tblDetalleTraslado.getValueAt(c++, 0) != null) {
                try {
                    ...
                    if ((sub_total - sub_total_actual) != 0) {
                        Mensaje.manipulacionExcepciones("critico", "Se encontro un error en la fila " + c, "Guardar");
                        error_detalle++;
                    }
        ...
        if (error_detalle > 0) {
            Mensaje.manipulacionExcepciones("informacion", "Revise el detalle, una o mas filas no fueron actualizadas", "Guardar");
        }
        ...
        if (txtIdTraslado.getText().equals("") && error_detalle == 0) {
            ...
            try {
                ...
                traslado.setFecha_trasaldo(ConvertirFecha(txtFecha));
                traslado.setTotal_traslado(Float.parseFloat(txtTotal.getText()));
                traslado.setUsuario_traslado(Inventory.lblUsuario.getText());
                acceso_traslado.insertarTraslado(traslado, Inventory.lblUsuario.getText(), Inventory.lblTerminal.getText());
            ...
            try {
                int id_traslado = acceso_traslado.obtenerIdTraslado(traslado, Inventory.lblUsuario.getText(), Inventory.lblTerminal.getText());
            ...
            if (traslado.getId_traslado() == 0) {
                Mensaje.manipulacionExcepciones("critico", "No se pudo obtener el ID de este traslado.", "Guardar");
            } else {
                    ...
                    for (int cFilas = 0; cFilas < tblDetalleTraslado.getRowCount(); cFilas++) {
                        ...
                        if (tblDetalleTraslado.getValueAt(cFilas, 0) != null) {
                            ...
                            try {
                                ...
                                acceso_detalle.insertarDetalleTraslado(detalle, Inventory.lblUsuario.getText(), Inventory.lblTerminal.getText());
                                btnGuardar.setEnabled(false);
                            } catch (Exception error) {
                                error_guardar_detalle = true;
                                Mensaje.manipulacionExcepciones("critico", "Ocurrio un error en la Fila " + cFilas, "Guardar");
                            }
                ...
                if (error_guardar_encabezado == false && error_guardar_detalle == false) {
                    ...
                    for (int cFilas = 0; cFilas < tblDetalleTraslado.getRowCount(); cFilas++) {
                        if (tblDetalleTraslado.getValueAt(cFilas, 0) != null) {
                            ...
                            acceso_producto.disminuirExistencia(id, cantidad, Inventory.lblUsuario.getText(), Inventory.lblTerminal.getText());
                        }
                    }

                    Mensaje.manipulacionExcepciones("informacion", "Traslado guardado con exito", "Guardar");
                }

            }
            
        } else {
            Mensaje.manipulacionExcepciones("critico","Este traslado ya fue guardada con anterioridad.", "Guardar");
        } 
        ...
        if (evt.getKeyCode() == 9) {
            ...
            if (columna_seleccionada == 4) {
                ...
                if (tblDetalleTraslado.getValueAt(fila_seleccionada, columna_seleccionada - 1) == null) {
                    Mensaje.manipulacionExcepciones("critico", "No ingreso la cantidad se produjo un error critico.", "Detalle Traslado");
                } else {
                ...
                if ((float) tblDetalleTraslado.getValueAt(fila_seleccionada, columna_seleccionada - 1) < 0.01) {
                    Mensaje.manipulacionExcepciones("critico", "La cantidad ingresada no es valida.", "Detalle Traslado");
                }
                ...
                if ((float) tblDetalleTraslado.getValueAt(fila_seleccionada, columna_seleccionada - 1) > Producto.getExi_producto()) {
                    Mensaje.manipulacionExcepciones("critico", "Esta cantidad excede el total de existencia.", "Detallle Traslado");
                }

                //EVALUAR EL MINIMO DE PRODUCTO EXISTENTE
                if (Producto.getExi_producto() <= Producto.getMinimo_producto()) {
                    Mensaje.manipulacionExcepciones("informacion", "Este producto a llegado al minimo.", "Detalle Traslado");
                }

                //EVALUAR SI EL PRODUCTO ESTA EN CERO
                if (Producto.getExi_producto() < 0.01) {
                    Mensaje.manipulacionExcepciones("informacion", "Este producto esta en cero.", "Detalle Traslado");
                }
    ...
    private void btnNuevoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNuevoActionPerformed
        ...
        int x = (ancho / 2) - (Traslado.getWidth() / 2);
        int y = (alto / 2) - (Traslado.getHeight() / 2);
        Traslado.setVisible(true);
        Inventory.pnlPrincipal.add(Traslado);
        Traslado.toFront();
        Traslado.setLocation(x, y);
    ...
    private void btnImprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnImprimirActionPerformed
        ...
        try{
            ...
            Mensaje.manipulacionExcepciones("critico", error.getMessage(), "Reporte de Traslados");
        }
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        ...
        try{
            ...
            Mensaje.manipulacionExcepciones("critico", "Se ha eliminado una fila.", "Eliminar");
        }
